...
test('mapped list mutations add and remove DOM rows in order', async () => {
  ...
  try {
    ...
    const rowTexts = () => Array.from(component.el.querySelectorAll('li')).map((node) => node.textContent?.trim())
...
test('getter-derived imported child list renders items after add', async () => {
  ...
  try {
    ...
    const store = new Store({
      ...
      nextId: 1,
    }) as Store & { addTodo: (text: string) => void; filteredTodos: Array<{ id: number; text: string; done: boolean }> }
