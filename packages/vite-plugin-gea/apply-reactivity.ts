        ...
        ClassDeclaration(classPath: NodePath<t.ClassDeclaration>) {
          ...
          const mergeObserveMethod = (observeKey: string, method: t.ClassMethod) => {
            ...
            if (existing && t.isBlockStatement(existing.body) && t.isBlockStatement(method.body)) {
              ...
              existing.body.body.push(...alignMethodBodyParams(method, existing.params))
              return
            }
          ...
          for (const pb of analysis.propBindings) {
            ...
            if (pb.expression && pb.setupStatements) {
              const rewrittenSetup = replacePropRefsInStatements(pb.setupStatements, templatePropNames)
              const rewrittenExpr = replacePropRefsInExpression(pb.expression, templatePropNames)
          ...
          analysis.unresolvedMaps.forEach((um, idx) => {
            ...
            if (handlerPropsInMap.length > 0 && um.computationExpr) {
              if (arrayPropName) {
                const propNames = getTemplatePropNames(classPath.node.body)
                const populateMethod = buildPopulateItemHandlersMethod(arrayPropName, handlerPropsInMap, propNames)
          ...
          for (const [observeKey, propPath] of stateProps) {
            ...
            if (!childObserveGroups.has(observeKey) && !stateChildObserveKeys.has(observeKey)) {
          ...
          unresolvedBindings.forEach(({ info, binding }) => {
            ...
            if (hasNonRelationalDeps) {
              mapRegistrations.push(generateMapRegistration(binding, info, getTemplatePropNames(classPath.node.body)))
              applied = true
            }
          ...
          if ((analysis.conditionalSlots || []).length > 0) {
            const templatePropNames = getTemplatePropNames(classPath.node.body)
            generateConditionalPatchMethods(classPath.node.body, analysis.conditionalSlots!, templatePropNames)
          }
          ...
          if (applied) {
            ...
            addedMethods.forEach((_method, observeKey) => {
              ...
              if (parts.length === 1) {
                ...
                if (storeRef?.getterDeps?.has(parts[0])) return
              }
...
function generateUnresolvedRelationalObserver(
  ...
  const containerLookup =
    arrayMap.containerBindingId !== undefined
      ...
      : (jsExpr`this.$(${arrayMap.containerSelector})` as t.Expression)
  ...
  const arrExpr = unresolvedMap.computationExpr
    ? replacePropRefsInExpression(t.cloneNode(unresolvedMap.computationExpr, true) as t.Expression, templatePropNames)
    : t.arrayExpression([])
...
function generateMapRegistration(
  ...
  if (templatePropNames && templatePropNames.size > 0) {
    arrExpr = replacePropRefsInExpression(arrExpr, templatePropNames)
  ...
  return t.expressionStatement(
    t.callExpression(t.memberExpression(t.thisExpression(), t.identifier('__geaRegisterMap')), [
      ...
      t.arrowFunctionExpression(
        [t.identifier('__item')],
        t.callExpression(t.memberExpression(t.thisExpression(), t.identifier(createMethodName)), [
          t.identifier('__item'),
        ]),
      ),
    ]),
  ...
  const patchCalls = Array.from(inlinePatchBodies.entries()).map(([propName, bodyStmts]) =>
    t.ifStatement(
      ...
      t.ifStatement(
        ...
        bodyStmts.length === 1 ? bodyStmts[0] : t.blockStatement(bodyStmts),
      ),
    ),
  )
...
function generateConditionalPatchMethods(
  ...
  const rewrittenCondExprs = slots.map((s) =>
    replacePropRefsInExpression(t.cloneNode(s.conditionExpr, true), templatePropNames),
  )
  const initSetup = pruneDeadParamDestructuring(
    replacePropRefsInStatements(
      allSetupStatements.map((s) => t.cloneNode(s, true) as t.Statement),
      templatePropNames,
    ),
    rewrittenCondExprs,
  )
  ...
  for (let i = 0; i < slots.length; i++) {
    const slot = slots[i]
    const rewrittenCondExpr = replacePropRefsInExpression(t.cloneNode(slot.conditionExpr, true), templatePropNames)
    ...
    const buildHtmlFn = (htmlExpr?: t.Expression): t.Expression => {
      ...
      const clonedHtmlExpr = replacePropRefsInExpression(t.cloneNode(htmlExpr, true), templatePropNames)
...
function replaceInlineMapWithRenderCall(
  ...
  arrayMap: { arrayPathParts: PathParts; itemVariable: string },
  ...
  traverse(tempProg, {
    ...
    CallExpression(path: NodePath<t.CallExpression>) {
      ...
      arrowFn.body = t.callExpression(t.memberExpression(t.thisExpression(), t.identifier(renderMethodName)), [
