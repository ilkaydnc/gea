...
export function generateArrayRelationalObserver(
  ...
  const idProp = bindings[0]?.itemIdProperty || arrayMap.itemIdProperty || 'id'
  ...
  const rawArrVar = t.identifier('__rawArr')
  const arrExpr = rawArrVar
...
function lazyInit(name: string, selector: string, bindingId?: string): t.Statement {
  ...
  return js`
    if (!this.${id(name)}) {
      this.${id(name)} =
        this.$(${selector}) ||
        (this.element_ && this.element_.matches && this.element_.matches(${selector})
          ? this.element_
          : null);
    }
  `
