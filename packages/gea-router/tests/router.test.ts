...
async function loadModules() {
  ...
  return { GeaRouter: GeaRouter as typeof import('../src/router').GeaRouter }
}

// Fake component classes for testing
class Home { static _name = 'Home' }
class About { static _name = 'About' }
class UserProfile { static _name = 'UserProfile' }
class NotFound { static _name = 'NotFound' }
class Dashboard { static _name = 'Dashboard' }
class LoginPage { static _name = 'LoginPage' }
class AdminPanel { static _name = 'AdminPanel' }
class MainLayout { static _name = 'MainLayout' }
class ProjectDetail { static _name = 'ProjectDetail' }
...
describe('GeaRouter', () => {
  ...
  it('back(), forward(), go() call history API', async () => {
    ...
    window.history.forward = () => { calls.push('forward') }
    window.history.go = (n: number) => { calls.push(`go:${n}`) }
  ...
  it('failed lazy routes set router.error', async () => {
    ...
    const FailingLazy = async () => { throw new Error('Network error') }
