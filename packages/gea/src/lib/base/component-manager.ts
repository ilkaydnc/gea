...
export default class ComponentManager {
  ...
  getParentComps(child: HTMLElement): ComponentLike[] {
    let node = child,
      comp,
      ids
    const parentComps = []
  ...
  callEventsGetterHandler(comp: ComponentLike, e: Event): any {
  ...
  callItemHandler(comp: ComponentLike, e: Event): any {
    if (!comp) return true
    ...
    if (itemEl && comp.el && comp.el.contains(itemEl) && typeof comp.__handleItemHandler === 'function') {
