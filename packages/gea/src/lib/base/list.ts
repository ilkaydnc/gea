...
function rebuildList(container: HTMLElement, array: any[], create: (item: any) => HTMLElement): void {
...
function rerenderListInPlace(container: HTMLElement, array: any[], create: (item: any) => HTMLElement): void {
...
function applyPropChanges(container: HTMLElement, items: any[], changes: StoreChange[], config: ListConfig): boolean {
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    const key =
      change.leafPathParts && change.leafPathParts.length > 0 ? change.leafPathParts.join('.') : change.property
    ...
    for (let j = 0; j < patchers.length; j++) {
      patchers[j](row, change.newValue, items[change.arrayIndex])
    }
  }
...
export function applyListChanges(
  ...
  if (firstChange?.type === 'update' && samePathParts(firstChange.pathParts, config.arrayPathParts)) {
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (change.type === 'delete') {
      deleteIndexes.push(Number(change.property))
      handledMutation = true
      continue
    }

    if (change.type === 'add') {
      addIndexes.push(Number(change.property))
      handledMutation = true
      continue
    }
    ...
    if (change.type === 'append') {
      ...
      if (count > 0) {
        ...
        for (let j = 0; j < count; j++) {
          fragment.appendChild(config.create(items[start + j]))
        }
        container.appendChild(fragment)
      }
      handledMutation = true
  ...
  for (let i = 0; i < addIndexes.length; i++) {
    ...
    const row = config.create(items[index])
