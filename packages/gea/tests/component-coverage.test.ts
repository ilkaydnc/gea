...
describe('Component – proxy props constructor branch', () => {
  let restoreDom: () => void
  let Component: any
  let Store: any
  ...
  it('accepts proxy props from a store', () => {
    const propsStore = new Store({ color: 'red' })
    class A extends Component {
      template() {
        return '<div></div>'
      }
    }
    const a = new A(propsStore)
    assert.equal(a.props.color, 'red')
    assert.ok(a.__propsStore)
  })

  it('creates __propsStore when __onPropChange is defined', () => {
    class B extends Component {
      __onPropChange(_prop: string, _val: any) {}
      template() {
        return '<div></div>'
      }
    }
    const b = new B({ x: 1 })
    assert.ok(b.__propsStore)
    assert.equal(b.props.x, 1)
  })

  it('__onPropChange is called when rendered and prop changes', async () => {
    ...
    c.__propsStore.x = 99
    await flush()
    assert.ok(calls.some(([p]) => p === 'x'))
  })

  it('__onPropChange skipped when not yet rendered', async () => {
    const calls: any[] = []
...
describe('Component – __geaCloneItem', () => {
  ...
  it('applies patches to cloned items', () => {
    ...
    c.render(container)
    const list = document.createElement('ul')
    const renderFn = (item: any) => `<li><span>text</span></li>`
    const patches = [
      [[0], 'c', 'highlight'],
      [[0], 't', 'new text'],
    ]
...
describe('Component – __reactiveProps', () => {
  ...
  it('creates reactive proxy from plain object', () => {
    class A extends Component {
      template() {
        return '<div></div>'
      }
    ...
    const a = new A()
    const reactive = a.__reactiveProps({ x: 1, y: 2 })
    assert.equal(reactive.x, 1)
    assert.equal((reactive as any).__isProxy, true)
  })
})
