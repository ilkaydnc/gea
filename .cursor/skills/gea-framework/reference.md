...
The `vite-plugin-gea` plugin handles:
...
```json
{
  ...
  "devDependencies": {
    "vite": "^7.3.1",
    "vite-plugin-gea": "^1.0.0"
  }
}
```

For mobile apps using navigation and gestures, also add `gea-mobile`:
...
When developing within the gea monorepo, alias `gea` to the source for live reloading:
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      gea: path.resolve(__dirname, '../../packages/gea/src')
    }
  }
})
```
